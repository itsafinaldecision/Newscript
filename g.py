...
bot = telebot.TeleBot('7301680961:AAGx_3U7FtMw5lDPBSrct4-Fg_KfiGVDzWQ')
# Owner and admin user IDs
owner_id = "7221383234"
admin_ids = ["7221383234"]
...
def send_welcome(message):
    response = (
        f"🌟 Welcome to the Anjaney VIP DDOS Bot! 🌟\n\n"
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_ids and user_id != owner_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 3:
                response = "You Are On Cooldown ❌. Please Wait 3sec Before Running The /attack Command Again."
                bot.reply_to(message, response)
                return
        ...
        response = ("🚫 Unauthorized Access! 🚫\n\nOops! It seems like you don't have permission to use the /attack command. "
                    ...
                    "🌟THE ONLY OWNER IS @AnjaneyHacks DM TO BUY ACCESS")
...
def show_all_users(message):
    user_id = str(message.chat.id)
    if user_id in ain_ids or owner_id:
        try:
            with open(USER_FILE, "r") as file:
